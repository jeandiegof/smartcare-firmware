...
static uint32_t custom_value_char_add2(ble_cus_t *p_cus, const ble_cus_init_t *p_cus_init)
{
    ...
    ble_uuid.type = p_cus->uuid_type;
    ble_uuid.uuid = CUSTOM_VALUE_CHAR_UUID + 1;
...
static void on_connect(ble_cus_t *p_cus, ble_evt_t const *p_ble_evt)
{
    ...
    ble_cus_evt_t evt = {.evt_type = BLE_CUS_EVT_CONNECTED};
...
static void on_disconnect(ble_cus_t *p_cus, ble_evt_t const *p_ble_evt)
{
    ...
    ble_cus_evt_t evt = {.evt_type = BLE_CUS_EVT_DISCONNECTED};
    if (p_cus->evt_handler != NULL)
    {
        p_cus->evt_handler(p_cus, &evt);
    }
...
static void on_write(ble_cus_t *p_cus, ble_evt_t const *p_ble_evt)
{
    ...
    if (p_evt_write->handle == p_cus->custom_value_handles.value_handle ||
        p_evt_write->handle == p_cus->char_b_handles.value_handle)
    {
        NRF_LOG_INFO("this was toggled by %d", p_evt_write->handle);
